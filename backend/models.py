...
class UserManager(BaseUserManager):
    """
    Миксин для управления пользователями (вход по email)
    """
...
class User(AbstractUser):
    """
    Кастомная модель пользователя, где Email является логином
    """
    REQUIRED_FIELDS = []
    objects = UserManager()
    ...
    is_active = models.BooleanField(
        _('active'),
        default=False,  # По умолчанию не активен, пока не подтвердит email
...
class Shop(models.Model):
    ...
    user = models.OneToOneField(User, verbose_name='Владелец (админ магазина)',
...
class ProductInfo(models.Model):
    """
    Связующее звено: продукт в конкретном магазине с конкретной ценой
    """
...
class ProductParameter(models.Model):
    """
    Характеристики конкретного товара (цвет, размер и т.д.)
    """
